...
RectangleModel::RectangleModel(GLFWWindowFactory* window):window(window){
    ...
    directionalLights = loadDirectionalLights("D:\\openGLProject\\lab\\src\\medium\\2.3objLoader\\dependencies\\config\\directionalLights.yaml");
    pointLights = loadPointLights("../dependencies/config/pointLights.yaml");
    modelInfos = loadScene("config/scene.yaml");
     
    for(auto& modelInfo : modelInfos){
        modelInfo.model = new Model(modelInfo.path);
    }
    shader = Shader("D:/openGLProject/lab/src/medium/2.3objLoader/dependencies/shader.vs", "D:/openGLProject/lab/src/medium/2.3objLoader/dependencies/shader.fs");
...
std::vector<RectangleModel::ModelInfo> RectangleModel::loadScene(const std::string& fileName) {
    ...
    try {
        ...
        std::cerr << "Error: Unable to open file " << fileName << ". Details: " << e.what() << std::endl;
    } catch(const YAML::ParserException& e) {
        std::cerr << "Error: Parsing failed: " << e.what() << ". Details: " << e.what() << std::endl;
    }
